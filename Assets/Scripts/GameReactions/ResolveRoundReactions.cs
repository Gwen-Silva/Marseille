...
public class ResolveRoundReactions : MonoBehaviour
{
	...
	private void ResolveRoundReaction(ResolveRoundGA ga)
	{
		...
		Debug.Log($"[ResolveReaction] Valor do Jogador: {playerValue}");
		Debug.Log($"[ResolveReaction] Valor do Oponente: {opponentValue}");

		int difference = playerValue - opponentValue;
		Debug.Log($"[ResolveReaction] Diferença calculada: {difference}");
