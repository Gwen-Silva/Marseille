...
public class ResolveRoundReactions : MonoBehaviour
{
	...
	private void ResolveRoundReaction(ResolveRoundGA ga)
	{
		...
		actionSystem.AddReaction(new CardCombatAnimationGA(attacker, defender, result));
		actionSystem.AddReaction(new WaitGA(DelayType.Medium));
		...
		if (result == CombatResult.AttackSuccess && difference > 0)
		{
			...
			actionSystem.AddReaction(new WaitGA(DelayType.Short));
		}

		actionSystem.AddReaction(new WaitGA(DelayType.Short));
		actionSystem.AddReaction(new ClearBoardGA());
		actionSystem.AddReaction(new WaitGA(DelayType.Short));
		actionSystem.AddReaction(new ToggleTurnOwnerGA());
		actionSystem.AddReaction(new WaitGA(DelayType.Short));
	...
	private void ApplyDamage(CardDisplay playerCard, CardDisplay opponentCard, int dmg, bool playerIsAttacking)
	{
		...
		{
			actionSystem.AddReaction(new DealDamageGA(playerCard, opponentCard, opponentHealth, dmg));
		}
		else
		{
			actionSystem.AddReaction(new DealDamageGA(opponentCard, playerCard, playerHealth, dmg));
		}
	}
	...
	private void TryAddGriefNullify(Card card, bool targetIsPlayer)
	{
		if (card != null && card.cardData.cardEffect == CardEffect.Grief)
		{
			...
			if (tier >= 1 && tier <= MAX_NULLIFIABLE_TIER)
				actionSystem.AddReaction(new GriefNullifyEffectGA(tier, targetIsPlayer));
		}
	}
	...
	private void TryAddGuiltDebuff(Card card, bool targetIsPlayer)
	{
		...
		{
			int tier = CardEffectUtils.GetTier(card.cardData.cardValue);
			actionSystem.AddReaction(new GuiltApplyDebuffGA(tier, targetIsPlayer));
		}
	}
	...
	private void TriggerEffect(Card effectCard)
	{
		...
		switch (effectCard.cardData.cardEffect)
		{
			case CardEffect.Love:
				actionSystem.AddReaction(new LoveGA(effectCard));
				break;
			case CardEffect.Grief:
				actionSystem.AddReaction(new GriefGA(effectCard));
				break;
			case CardEffect.Guilt:
				actionSystem.AddReaction(new GuiltGA(effectCard));
				break;
			case CardEffect.Doubt:
				actionSystem.AddReaction(new DoubtGA(effectCard));
				break;
		}
	}
